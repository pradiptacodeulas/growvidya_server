...
class MessageController {
  ...
  static async deleteMessage(req, res, next) {
    try {
      const { userId, role, schoolId } = MessageController.extractUserInfo(req);
      const messageId = Number(req.params.id || req.body.messageId);

      if (!messageId) {
        return ApiResponse.error(res, 'Message ID is required.', null, 400);
      }
      ...
      if (!result.success) {
        if (result.notFound) {
          return ApiResponse.error(res, result.message, null, 404);
        }
        if (result.forbidden) {
          return ApiResponse.error(res, result.message, null, 403);
        }
        return ApiResponse.error(res, result.message, null, 400);
      }

      // Broadcast real-time deletion event to both sender and receiver
      try {
        const io = getIO();
        ...
        console.warn('[Socket message_deleted broadcast error]:', socketErr.message);
      }

      return ApiResponse.success(res, 'Message deleted successfully.', { messageId });
    } catch (error) {
      next(error);
    }
  }
