...
class TeacherAuthController {
  ...
  static async _issueSession(teacher, res, message = 'Teacher authentication successful.') {
    ...
    // Fetch full teacher profile including joined relations
    const fullTeacher = (await TeacherModel.findAuthProfileById(teacher.id)) || teacher;
    const { password: _, passcode: __, otp: ___, ...safeTeacher } = fullTeacher;
  ...
  static _formatTeacherResponse(safeTeacher) {
    const rawAddresses =
      safeTeacher.address_list && safeTeacher.address_list.length > 0
        ? safeTeacher.address_list
        : safeTeacher.address_info
        ? [safeTeacher.address_info]
        : [];

    const addresses =
      rawAddresses.length > 0
        ? rawAddresses.map((addr, idx) => ({
            id: String(addr.id || idx + 1),
            school_id: String(addr.school_id || safeTeacher.school_id || ''),
            ...
            address_type: addr.address_type !== undefined ? Number(addr.address_type) : idx + 1,
            same_permanent: addr.same_permanent !== undefined ? Number(addr.same_permanent) : 1,
            status: addr.status !== undefined ? Number(addr.status) : 1,
          }))
        : [
            {
              id: String(safeTeacher.id || 1),
              ...
              postal_code: safeTeacher.postal_code ? String(safeTeacher.postal_code) : '',
              address_type: 1,
              same_permanent: 1,
              status: 1,
            },
    ...
    return {
      ...
      father_name: safeTeacher.father_name || '',
      mother_name: safeTeacher.mother_name || '',
      pan_number: safeTeacher.pan_number || '',
      language_known: safeTeacher.language_known || '',
      notes: safeTeacher.notes || '',
      previous_school_name: safeTeacher.previous_school_name || '',
      previous_school_address: safeTeacher.previous_school_address || '',
      previous_school_phone: safeTeacher.previous_school_phone || '',
      address1: safeTeacher.address1 || '',
      address2: safeTeacher.address2 || '',
      address: addresses,
  ...
  static async updateProfile(req, res, next) {
    try {
      ...
      const language_known = req.body.language_known;
      ...
      const permAddress1 = req.body.permanent_address_1 !== undefined ? req.body.permanent_address_1 : req.body.parmanent_address_1;
      const permAddress2 = req.body.permanent_address_2 !== undefined ? req.body.permanent_address_2 : req.body.parmanent_address_2;
      const permCountry = req.body.permanent_country !== undefined ? req.body.permanent_country : req.body.parmanent_country;
      const permState = req.body.permanent_state !== undefined ? req.body.permanent_state : req.body.parmanent_state;
      const permCity = req.body.permanent_city !== undefined ? req.body.permanent_city : req.body.parmanent_city;
      const permPostal = req.body.permanent_postal_code !== undefined ? req.body.permanent_postal_code : req.body.parmanent_postal_code;
      ...
      if (father_name !== undefined) {
        ...
        params.push(father_name ? String(father_name).trim() : null);
      }
      if (mother_name !== undefined) {
        updates.push('mother_name = ?');
        params.push(mother_name ? String(mother_name).trim() : null);
      }
      if (pan_number !== undefined) {
        updates.push('pan_number = ?');
        params.push(pan_number ? String(pan_number).trim() : null);
      }
      if (language_known !== undefined) {
        updates.push('language_known = ?');
        params.push(language_known ? String(language_known).trim() : null);
      }
      ...
      if (
        ...
        try {
          ...
          // Sync permanent address (address_type = 2) if samePermanent is 0
          if (samePermanent === 0) {
            const cleanPermCountry = permCountry !== undefined ? await resolveLocationId('countries', 'id', 'name', permCountry) : addrCountry;
            const cleanPermState = permState !== undefined ? await resolveLocationId('states', 'id_state', 'state', permState) : addrState;
            const cleanPermCity = permCity !== undefined ? await resolveLocationId('cities', 'id', 'name', permCity) : addrCity;
            let cleanPermPostal = addrPostal;
            if (permPostal !== undefined && permPostal !== null) {
              const pDigits = String(permPostal).replace(/\D/g, '');
              if (pDigits) cleanPermPostal = parseInt(pDigits.slice(0, 9), 10);
            }
            const pAddr1 = permAddress1 !== undefined ? (permAddress1 ? String(permAddress1).trim() : '') : addr1;
            const pAddr2 = permAddress2 !== undefined ? (permAddress2 ? String(permAddress2).trim() : '') : addr2;

            const [permRows] = await pool.query(
              'SELECT id FROM teacher_address WHERE teacher_id = ? AND address_type = 2 LIMIT 1',
              [teacherId]
            );
            if (permRows && permRows.length > 0) {
              await pool.query(
                `UPDATE teacher_address SET address1 = ?, address2 = ?, country = ?, state = ?, city = ?, postal_code = ?, same_permanent = 0 WHERE id = ?`,
                [pAddr1, pAddr2, cleanPermCountry, cleanPermState, cleanPermCity, cleanPermPostal, permRows[0].id]
              );
            } else {
              await pool.query(
