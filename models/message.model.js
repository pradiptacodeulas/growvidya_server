...
class MessageModel {
    ...
    const sql = `
      SELECT * FROM (
        SELECT 
          ...
          type,
          CASE 
            WHEN status = 0 THEN 'This message was deleted'
            ELSE CAST(message AS CHAR)
          END AS message,
          CASE 
  ...
  static async getContactsForUser({ school_id = 1, userId, role }) {
    ...
    const [latestMessages] = await pool.query(
      `SELECT 
        ...
        m.receiver_role,
        CASE 
          WHEN m.status = 0 THEN 'This message was deleted'
          ELSE CAST(m.message AS CHAR)
        END AS message,
        CASE 
          WHEN m.status = 0 THEN NULL
          ELSE m.file
        END AS file,
        m.file_type,
        m.time,
    ...
    for (const msg of latestMessages) {
      ...
      const previewText = msg.status === 0
        ? '🚫 This message was deleted'
        : (msg.message || (msg.file ? '📎 Attachment' : ''));
  ...
  static async deleteMessage({ school_id = 1, messageId, userId, role }) {
    ...
    if (!rows || rows.length === 0) {
      return { success: false, notFound: true, message: 'Message not found.' };
    }
    ...
    if (msg.status === 0) {
      return { success: false, alreadyDeleted: true, message: 'Message is already deleted.' };
    }

    // Permission check: Sender or School Admin
    const isSender =
      Number(msg.sender) === Number(userId) &&
      String(msg.sender_role).toLowerCase() === String(role).toLowerCase();
    const isAdmin = String(role).toLowerCase() === 'admin';

    if (!isSender && !isAdmin) {
      return { success: false, forbidden: true, message: 'You are not authorized to delete this message.' };
    }
