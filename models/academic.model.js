...
class AcademicModel {
    ...
    try {
      let sql = `
        SELECT r.*, cm.class_name, sec.section_name, sm.subject_name, 
               ...
               COALESCE(dm.day_name, CASE r.day 
                 WHEN 1 THEN 'Monday'
                 WHEN 2 THEN 'Tuesday'
  ...
  static async createRoutine(schoolId, { class_id, section_id, day, period_id, teacher_id, subject_id, shift_id = 1 }) {
    const [result] = await pool.query(
      `INSERT INTO routine (school_id, class_id, section_id, day, period_id, teacher_id, subject_id, shift_id, status, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, NOW())`,
      [schoolId, class_id, section_id, day, period_id, teacher_id || null, subject_id, shift_id]
    );
    return result.insertId;
  }
  ...
  static async updateRoutine(id, schoolId, { class_id, section_id, day, period_id, teacher_id, subject_id, shift_id }) {
    const [result] = await pool.query(
      ...
      [class_id, section_id, day, period_id, teacher_id || null, subject_id, shift_id || 1, id, schoolId]
    );
    return result.affectedRows > 0;
  }
