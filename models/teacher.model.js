...
class TeacherModel {
    ...
    try {
      ...
      const [usedLeaves] = await pool.query(
        `SELECT l.leave_id, 
         ...
         WHERE l.staff_id = ? AND (l.school_id = ? OR ? IS NULL) AND l.role = 1 AND ld.status = 2 AND l.status != 4 AND l.status != 0
         GROUP BY l.leave_id`,
      ...
      (usedLeaves || []).forEach((u) => {
        usedMap[u.leave_id] = Number(u.approved_days || 0);
      });
  ...
  static async updateTeacher(id, schoolId, data) {
    ...
    try {
      if (data.class_assignments !== undefined && Array.isArray(data.class_assignments)) {
        // Fetch existing active assignments before soft-deleting
        const [existingAssigns] = await pool.query(
          `SELECT DISTINCT class_id, subject_id FROM teacher_class_assign 
           WHERE teacher_id = ? AND school_id = ? AND status IN (1, 2)`,
          [id, schoolId]
        );
        ...
        // Unassign teacher from routine slots for any removed class or subject assignments
        const newClassIds = new Set(uniqueAssignments.map((u) => Number(u.class_id)).filter(Boolean));
        for (const prev of existingAssigns) {
          const prevClassId = Number(prev.class_id);
          const prevSubjectId = Number(prev.subject_id || 0);
          const pairKey = `${prevClassId}-${prevSubjectId}`;

          // If the specific (class_id, subject_id) pair was removed
          if (!seenPairs.has(pairKey)) {
            if (prevSubjectId > 0) {
              await pool.query(
                `UPDATE routine SET teacher_id = NULL 
                 WHERE school_id = ? AND teacher_id = ? AND class_id = ? AND subject_id = ? AND (status != 4 OR status IS NULL)`,
                [schoolId, id, prevClassId, prevSubjectId]
              );
            }
          }

          // If the entire class is no longer assigned to this teacher
          if (!newClassIds.has(prevClassId)) {
            await pool.query(
              `UPDATE routine SET teacher_id = NULL 
               WHERE school_id = ? AND teacher_id = ? AND class_id = ? AND (status != 4 OR status IS NULL)`,
              [schoolId, id, prevClassId]
            );
          }
        }
      } else {
    ...
    try {
      const [addrRows] = await pool.query(
        `SELECT ta.*, co.name AS country_name, st.state AS state_name, cit.name AS city_name
         ...
         LEFT JOIN cities cit ON ta.city = cit.id
         WHERE ta.teacher_id = ?
         ORDER BY ta.address_type ASC, ta.id ASC`,
        [teacherId]
      );
      if (addrRows && addrRows.length > 0) {
        const currentAddr = addrRows.find((a) => Number(a.address_type) === 1) || addrRows[0];
        const permAddr = addrRows.find((a) => Number(a.address_type) === 2) || (Number(currentAddr.same_permanent) === 1 ? currentAddr : null);
        teacher.address_info = currentAddr;
        teacher.address_list = [currentAddr, permAddr].filter(Boolean);
        teacher.address1 = currentAddr.address1 || teacher.address1 || '';
        teacher.address2 = currentAddr.address2 || teacher.address2 || '';
        teacher.country = currentAddr.country || teacher.country || null;
        teacher.country_name = currentAddr.country_name || '';
        teacher.state = currentAddr.state || teacher.state || null;
        teacher.state_name = currentAddr.state_name || '';
        teacher.city = currentAddr.city || teacher.city || null;
        teacher.city_name = currentAddr.city_name || '';
        teacher.postal_code = currentAddr.postal_code || teacher.postal_code || '';
      } else {
        teacher.address_list = [];
        if (teacher.country) {
          const [cRows] = await pool.query('SELECT name FROM countries WHERE id = ?', [teacher.country]);
          if (cRows && cRows.length > 0) teacher.country_name = cRows[0].name;
        }
        if (teacher.state) {
          const [sRows] = await pool.query('SELECT state FROM states WHERE id_state = ?', [teacher.state]);
          if (sRows && sRows.length > 0) teacher.state_name = sRows[0].state;
        }
        if (teacher.city) {
          const [ctRows] = await pool.query('SELECT name FROM cities WHERE id = ?', [teacher.city]);
          if (ctRows && ctRows.length > 0) teacher.city_name = ctRows[0].name;
        }
      }
    } catch (e) {
  ...
  static async deleteTeacher(id, schoolId) {
    ...
    // Unassign teacher from any active routine timetable slots
    try {
      await pool.query(
        `UPDATE routine SET teacher_id = NULL WHERE teacher_id = ? AND school_id = ? AND (status != 4 OR status IS NULL)`,
        [id, schoolId]
      );
    } catch (e) {
      console.error('Error unassigning teacher from routines:', e.message);
    }
