...
class BranchModel {
  ...
  static async getAllBranches({ school_id = 1, status = null }) {
    ...
    sql += ` ORDER BY is_main_branch DESC, branch_name ASC`;
  ...
  static async createBranch({
    ...
    const sql = `
      INSERT INTO branch_master (
        school_id, branch_name, branch_code, address, city, state, pincode,
        phone, email, principal_name, is_main_branch, status
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
  ...
  static async updateBranch(id, school_id = 1, data = {}) {
    ...
    const [result] = await pool.execute(sql, [
      ...
      status !== undefined ? (Number(status) === 0 ? 0 : 1) : null,
      Number(id),
      Number(school_id) || 1,
    ]);
  ...
  /**
   * Delete a branch with safety check
   */
  static async deleteBranch(id, school_id = 1) {
    ...
    const [result] = await pool.execute(
      `DELETE FROM branch_master WHERE id = ? AND school_id = ?`,
